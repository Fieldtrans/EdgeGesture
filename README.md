...
App 本身主要负责设置参数。普通模式通过 Android 无障碍服务创建边缘触发区；增强模式会把手势监听运行在模块注入的 `system_server` 中，所以模块加载后，即使杀掉 App 进程，手势也应该继续可用。

LSPosed 增强方案保留 InputFilter / system_server 监听，Overlay 只显示不吃触摸；只有确认进入指针模式后才消费事件，横向侧滑会尽早放行。
...
The app mainly saves settings. Standard mode uses Android Accessibility to create edge trigger regions. Enhanced mode injects the gesture listener into `system_server` through LSPosed/Xposed, so gestures can keep working after the app process is killed.

The enhanced design keeps the overlay visual-only and avoids using a transparent full-screen touch layer. This reduces conflicts with normal touches and system back gestures.
