...
class ConfigRepository(private val context: Context) {
     ...
     */
    suspend fun loadSettings(): SettingsState = withContext(Dispatchers.IO) {
     ...
     */
    suspend fun saveSettings(state: SettingsState) = withContext(Dispatchers.IO) {
     ...
     * Load hook status
     */
    suspend fun loadHookStatus(): HookStatus = withContext(Dispatchers.IO) {
        val moduleLoadedInApp = HookHealth.isModuleLoaded()

        val activeStatusPrefs = if (deviceStatusPrefs.contains(GestureConfig.KEY_STATUS_LOADED_AT) ||
